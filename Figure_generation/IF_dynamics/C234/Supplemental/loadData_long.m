...
settings.conditions = {
        ...
        'GM Cdt1',2:3,2:3,1:6,[0 0 0],[]; %1    
        'Thy/Aphi Cdt1',4:5,2:3,1:6,[0 0 0],[]; %1  
        'Thy/Thy Cdt1',6:7,2:3,1:6,[0 0 0],[]; %1  
        
        'GM GMNN',2:3,4:5,1:6,[0 0 0],[]; %1    
        'Thy/Aphi GMNN',4:5,4:5,1:6,[0 0 0],[]; %1  
        'Thy/Thy GMNN',6:7,4:5,1:6,[0 0 0],[]; %1 
        
        'GM CCNA',2:3,6:7,1:6,[0 0 0],[]; %1    
        'Thy/Aphi CCNA',4:5,6:7,1:6,[0 0 0],[]; %1  
        'Thy/Thy CCNA',6:7,6:7,1:6,[0 0 0],[]; %1 
        
        'GM PCNA',2:3,8:9,1:6,[0 0 0],[]; %1    
        'Thy/Aphi PCNA',4:5,8:9,1:6,[0 0 0],[]; %1  
        'Thy/Thy PCNA',6:7,8:9,1:6,[0 0 0],[]; %1 
    };

%%% Paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.dataDir = 'F:\Data\C-Cdt1\C234-live\Data';
...
settings.saveName = 'C234_data.mat';
settings.saveDir = 'F:\Data\C-Cdt1\C234-live\Plot';
%%% Analysis Options %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             inds = find(highNoise);
        ...
        if settings.poiCrl          
            ...
            S(i).POI(subset,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea(subset,:),...
                'traceStats', S(i).traceStats(subset,:),'nuc',S(i).crlNuc(subset,:), 'cyto', S(i).crlCyto(subset,:)), ...
                struct('cycling',1,'smooth',5 ,'buff',1 , 'postBuffer', 3 ,'postBufferSec',3,...
                'firstD',0,'secD',-.005,'decrease', .2, 'minSlope', -.05, 'early', 1,'falseCall', .8, 'cytoCheck', 0,'minExp',50,...
                'trunk',.4,'debug',0));
            ...
            if settings.poiG2
                [S(i).POI(:,5), badTracesCRL] = findCRLInact(S(i).crlNormAct, S(i).traceStats, S(i).POI(:,4), ...
                    ...
                    'buff',5,'thresh',.0025,'preBuffer',5,'lowThresh',.15,...
                    'increase',.005*5,'trunc',.75, 'medfilt',0,'early', 15,'debug',0));
            end
        end
