...
setting.saveName = 'C217_data.mat';
setting.dataDir = 'F:\Data\C-Cdt1\Figures\Paper\GMNN_dynamics\C217';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).YFP1med = S(i).IFdata(:, find(ismember(IFnames,'1_YFP_median')));
        S(i).FarRed1med = S(i).IFdata(:, find(ismember(IFnames,'1_FarRed_median')));
        
        S(i).YFP2 = S(i).IFdata(:, find(ismember(IFnames,'2_YFP_mean')));
        S(i).YFP3 = S(i).IFdata(:, find(ismember(IFnames,'3_YFP_mean')));
        S(i).YFP4 = S(i).IFdata(:, find(ismember(IFnames,'4_YFP_mean')));
        S(i).YFP5 = S(i).IFdata(:, find(ismember(IFnames,'5_FITC_mean')));
...
%         S(i) = gateout_all(S(i),~(badTracesAPC));
        ...
        if setting.poiApc
            if setting.quiescent
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    ...
                    'buff',1,'thresh',.4,'preBuffer',1,'lowThresh',20, 'increase',10*.4,'trunc',50, 'medfilt',0,'early', 25,'debug',0));
            else
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcAreaNormM, S(i).traceStats, ...
                    struct('postBuffer',5,'smooth',5,'cycling',1,...
    ...
    if ~isempty(setting.crl)
        ...
        if setting.poiCrl
            S(i).POI(:,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea,'traceStats', S(i).traceStats,'nuc',S(i).crlNuc, 'cyto', S(i).crlCyto), ...
                struct('cycling',~setting.quiescent,'smooth',5 ,'buff',1 , 'postBuffer', 3 ,'postBufferSec',1,...
                'firstD',0,'secD',-.005,'decrease', .05*2.5, 'minSlope', -.02, 'early', 5,'falseCall', .9, 'cytoCheck', 0,'minExp',75,...
                'trunk',.4,'debug',0));
            ...
            if setting.poiG2
                [S(i).POI(:,5), badTracesCRL] = findCRLInact(S(i).crlNormAct, S(i).traceStats, S(i).POI(:,4), ...
                    struct('postBuffer',5,'smooth',5,...
                    'buff',10,'thresh',.0025,'preBuffer',5,'lowThresh',.2, 'increase',.02,'trunc',.5, 'medfilt',0,'early', 25,'debug',0));
            end
        end
    ...
    if setting.IFoption        
        ...
        S(i).allYFP3 = allIFdata(:, find(ismember(IFnames,'3_YFP_mean')));
        S(i).allYFP4 = allIFdata(:, find(ismember(IFnames,'4_YFP_mean')));
        S(i).allYFP5 = allIFdata(:, find(ismember(IFnames,'5_FITC_mean')));
