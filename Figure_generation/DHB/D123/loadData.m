...
setting.saveName = 'D123_data.mat';
setting.dataDir = 'C:\Users\nalin\Documents\GitHub\image-analysis-ratnayeke-2022\Figure_generation\DHB\D123';
...
for i = 1:uniqueCondnum
                ...
                for site = siteMat
                    ...
                    if exist([dataDir,'traceData_',shot,'.mat']) && (~setting.IFoption || exist([dataDir,setting.IFlabel,shot,'.mat']))
                       ...
                       try
                        ...
                        wellindexTemp = ones(size(allIFtemp,1),3);
                        wellindexTemp(:,1) = wellindexTemp(:,1)*row;wellindexTemp(:,2) = wellindexTemp(:,2)*col;wellindexTemp(:,3) = wellindexTemp(:,3)*site;
                        allIFwell = [allIFwell; wellindexTemp];
    ...
    if setting.IFoption
        ...
        S(i).DAPI1 = IFdata(:, ismember(IFnames,'1_DAPImean'));
        S(i).YFP1 = IFdata(:, ismember(IFnames,'1_YFPmean'));
        S(i).YFP1cyt = IFdata(:, ismember(IFnames,'1_YFPcyto ring'));
        S(i).FarRed1 = IFdata(:, ismember(IFnames,'1_FarRedmean'));
        S(i).FarRed1cyt = IFdata(:, ismember(IFnames,'1_FarRedcyto ring'));
...
%                 S(i) = gateout_all(S(i),~(badTracesAPC));
            ...
            switch setting.type
                ...
                case 'cycling'
                    [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcAreaNormM, S(i).traceStats, ...
                        ...
                        'buff',0,'thresh',.002,'preBuffer',5,'lowThresh',.05, 'increase',.005 * 10,'trunc',.5, 'medfilt',0,'early', 5,'debug',0));
            end
            %S(i) = gateout_all(S(i),~(badTracesAPC));
    ...
    if setting.IFoption        
        ...
        S(i).allDAPI1 = allIFdata(:, find(ismember(IFnames,'1_DAPImean')));
        S(i).allDNA = S(i).allDAPI1 .* S(i).allIFarea;
        S(i).allYFP1 = allIFdata(:, find(ismember(IFnames,'1_YFPmean')));
        S(i).allYFP1cyt = allIFdata(:, find(ismember(IFnames,'1_YFPcyto ring')));
        S(i).allFarRed1 = allIFdata(:, find(ismember(IFnames,'1_FarRedmean')));
        S(i).allFarRed1cyt = allIFdata(:, find(ismember(IFnames,'1_FarRedcyto ring')));
        S(i).allIFwell = allIFwell;
    end
end
