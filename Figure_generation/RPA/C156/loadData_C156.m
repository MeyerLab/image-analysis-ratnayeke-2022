function [S] = loadData_C156(conditions, dataDir)
...
for i = 1:uniqueCondnum
    ...
    for n = 1:size(S(i).massNorm,1)
        highNoise(n,1) = any(abs(diff(S(i).massNorm(n,:))) > .15);
    end
    ...
    if ~isempty(setting.apc)
        S(i).apcNuc = S(i).traceData(:,:,ismember(names,[setting.apc '_median']));% - S(i).apcLocalBg;
        ...
        if setting.quiescentAnalysis
            ...
            S(i) = gateout_all(S(i),~(highMean | highNoise ));
        else
            % Cycling
            minMaxThresh = [10 100];
            minMaxNoise = [-500 500];
            highBG = 1000;
            traceGem = S(i).apcNuc;
            traceGembg = S(i).apcLocalBg;
            buff = 5;
            postMitosis = 1;
            [badTracesApc] = gateSigTraces(traceGem, traceGembg, S(i).traceStats, buff, postMitosis, minMaxThresh, minMaxNoise, highBG);
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact_C156(S(i).apcNuc, S(i).traceStats, ...
                    struct('postBuffer',8,'smooth',5,'cycling',0,...
                    'buff',1,'thresh',.7,'preBuffer',1,'lowThresh',25, 'increase',8*.7,'trunc',50, 'medfilt',0,'early', 30));
            else
                [S(i).POI(:,2), badTracesAPC] = findAPCInact_C118(S(i).apcNormM, S(i).traceStats, ...
                    struct('postBuffer',5,'smooth',3,'cycling',1,...
                    'buff',1,'thresh',.9,'preBuffer',1,'lowThresh',.1, 'increase',.025,'trunc',.5, 'medfilt',5,'early', 30));
            end
             S(i) = gateout_all(S(i),~(badTracesAPC));
        end
    ...
    if ~isempty(setting.crl)
        ...
        if setting.poiCrl
            S(i).POI(:,3) = findCRL4Act_C118(S(i).crlAreaNorm,S(i).traceStats, ...
                struct('cycling',~setting.quiescentAnalysis,'low',.1,'smooth',5, ...
                'firstD',-.02,'secD',-.01,'early', 0,'falseCall', .8,'buff',0, 'postBuffer', 2,'decrease', .01));
...
%         S(i).varIntensityNorm = S(i).varIntensity./repmat(nanmean(S(i).pcnaNuc(:,meanWindow),2),[1 size(S(i).pcnaNuc,2)]);
    ...
    if setting.IFoption
        ...
        S(i).YFP1cyt = S(i).IFdata(:, find(ismember(IFnames,'1_YFP_cyto ring')));
        S(i).YFP1bg = S(i).IFdata(:, find(ismember(IFnames,'1_YFP_block bg')));
        S(i).YFP2cyt = S(i).IFdata(:, find(ismember(IFnames,'2_YFP_cyto ring')));
        S(i).YFP2bg = S(i).IFdata(:, find(ismember(IFnames,'2_YFP_block bg')));
        
        S(i).FarRed1 = S(i).IFdata(:, find(ismember(IFnames,'1_FarRed_mean')));
        S(i).FarRed2 = S(i).IFdata(:, find(ismember(IFnames,'2_FarRed_mean')));
        S(i).FarRed1cyt = S(i).IFdata(:, find(ismember(IFnames,'1_FarRed_cyto ring')));
        S(i).FarRed1bg = S(i).IFdata(:, find(ismember(IFnames,'1_FarRed_block bg')));
        S(i).FarRed2cyt = S(i).IFdata(:, find(ismember(IFnames,'2_FarRed_cyto ring')));
        S(i).FarRed2bg = S(i).IFdata(:, find(ismember(IFnames,'2_FarRed_block bg')));
...
for i = 1:length(S)
    inds = S(i).IFarea > 4000;
    S(i) = gateout_all(S(i),~( inds));

end


save([dataDir setting.saveName],'S','-v7.3');
