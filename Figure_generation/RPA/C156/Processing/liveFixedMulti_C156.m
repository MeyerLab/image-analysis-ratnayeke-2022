function liveFixedMulti_C156( row,col,site, debug_mode, logFolder, filePrefix)
% row=2;
% col=2;
% site=4;
% debug_mode = 0;
...
%%% Paths
settings.data_path = 'F:\Data\C-Cdt1\C156-live\Data\';
settings.IF_imagesessions = {'G:\4TB7\Data\C156-IF1\', 'G:\4TB7\Data\C156-IF2\'};
settings.live_imagepath = 'G:\4TB7\Data\C156-live\Raw\';
settings.bgcmospath = 'F:\MATLAB\BGimages\IX_XL\cmosoffset_bin1.mat';
settings.crop_save = 'G:\4TB7\Data\C156-live\';
...
settings.postbin = [0 0]; %0 for no bin, number for scaling factor;
...
settings.ringcalc = {[1 0 1],[0 1 1]};
settings.ringthresh = {[100 100 100], [100 100 100]};
settings.punctacalc = {[0 0 0],[0 0 0]};
settings.punctaThresh = {{[],[],[]},{[],[]}};
settings.punctatopsize = 2;
settings.localbg = {[0 1 1],[0 1 1]};
...
settings.bgsubmethod = {{'global nuclear','global cyto', 'global cyto'}, ...
     {'global nuclear','global cyto', 'global cyto'}}; %'global nuclear','global cyto','tophat','semi-local nuclear'
...
settings.boulderarea = 10000; %1500 % less restrictive for C152 for >4N cells
...
if exist([settings.data_path,'tracedata_',shotLive,'.mat']) & ~exist([settings.data_path,'IF_',shotLive,'.mat'])
    ...
    try
        ...
        header = output_names_multiIF_puncta(settings.signals,settings.localbg,settings.ringcalc,settings.punctacalc,settings.punctaThresh);
...
if any(cell2mat(settings.ringcalc))
    if (settings.magnification == 10 & settings.binsize == 1) | ...
            (settings.magnification == 20 & (settings.binsize == 2 | any(settings.postbin)))
        innerrad = 1; outerrad = 5;
    else
        innerrad = 2; outerrad = 10;
    end
    ring_label = getcytoring_thicken(nuc_label,innerrad,outerrad,real_padded{1}{1});
