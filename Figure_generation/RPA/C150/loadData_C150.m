function [S] = loadData_C150(conditions, dataDir)
...
for i = 1:uniqueCondnum
    ...
    if ~isempty(setting.apc)
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact_C150(S(i).apcNuc, S(i).traceStats, ...
                    struct('postBuffer',8,'smooth',5,'cycling',0,...
                    'buff',1,'thresh',.75,'preBuffer',1,'lowThresh',25, 'increase',8*.75,'trunc',50, 'medfilt',0,'early', 50));
            else
