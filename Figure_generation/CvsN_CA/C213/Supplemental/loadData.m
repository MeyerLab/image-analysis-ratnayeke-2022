...
setting.saveName = 'C213_data.mat';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).FarRedPunctaArea = S(i).IFdata(:,contains(IFnames,'1_FarRed_puncta area'));
        S(i).FarRedPunctaIntensity = S(i).IFdata(:,contains(IFnames,'1_FarRed_puncta intensity'));
        S(i).FarRedPunctaNum = S(i).IFdata(:,contains(IFnames,'1_FarRed_puncta number'));
        ...
        if setting.poiApc
            if setting.quiescent
                ...
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcAreaNormM, S(i).traceStats, ...
                    struct('postBuffer',20,'smooth',9,'cycling',1,...
                    'buff',3,'thresh',.001,'preBuffer',5,'lowThresh',.05, 'increase',.01,'trunc',.5, 'medfilt',0,'early', 15,'debug',0));
            end
            %S(i) = gateout_all(S(i),~(badTracesAPC));
...
%         S(i).crlNuc = fillTraceVals(S(i).crlNuc, S(i).traceStats, 5);
        ...
        if setting.poiCrl
            S(i).POI(:,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea,'traceStats', S(i).traceStats,'nuc',S(i).crlNuc, 'cyto', S(i).crlCyto), ...
                struct('cycling',~setting.quiescent,'smooth',7 ,'buff',10 , 'postBuffer', 5 ,'postBufferSec',2,...
                'firstD',0,'secD',-.005,'decrease', .04*5, 'minSlope', -.02, 'early', 5,'falseCall', .9, 'cytoCheck', 200,'minExp',100,...
                'trunk',.4,'debug',0));
