function [S] = loadData_C134(conditions, dataDir)
...
%     S(i) = gateout_all(S(i),~(highNoise));
    ...
    if ~isempty(setting.cdk)
        ...
        maxThresh = 100; %threshold above which max of each trace must be  %150
        ...
        for n = 1:size(S(i).cdk,1)
            S(i).cdkSmooth(n,:) = nansmooth(S(i).cdk(n,:),smoothWindow);
        end
    end
...
%         end
    ...
    if ~isempty(setting.crl)
        S(i).crlNuc = S(i).traceData(:,:,ismember(names,[setting.crl '_median']));% - S(i).crlLocalBg;
%         S(i).crlNuc = correctBlankFrame(S(i).crlNuc, S(i).shot, 20:72, -50);
        S(i).crlNuc(:,76) = NaN;
        S(i).crlNuc = fillTraceVals3D(S(i).crlNuc, S(i).traceStats, 5);
        ...
        S(i) = gateout_all(S(i),~( highNoise | lowMean | lowTracestart));
        ...
        if setting.poiCrl
            S(i).POI(:,3) = findCRL4Act_C130(S(i).crlAreaNorm,S(i).traceStats, ...
                struct('cycling',~setting.quiescentAnalysis,'low',.1,'smooth',5, ...
                'firstD',-.01,'secD',-.005,'early', 30,'falseCall', .8,'buff',0, 'postBuffer', 2,'decrease', .01));
...
for i = 1:length(S)
    %S(i).YFP2corr = S(i).YFP2 - (.0034*S(i).YFP1 + 6.9);
end
