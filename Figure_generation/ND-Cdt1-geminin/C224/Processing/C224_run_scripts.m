...
%  nikon_rename('K:\C224_live\20200203_192150_907','K:\Nikon\C224-live\Raw','COPY',false)
% nikon_rename('K:\Nikon\C224_fixed\20200204_133431_561','K:\Nikon\C224-FP\Raw','COPY',false)
% nikon_rename('K:\Nikon\C224_IF1\20200205_152546_899','K:\Nikon\C224-IF1\Raw','COPY',false)
% nikon_rename('K:\Nikon\C224_IF2\20200206_125841_453','K:\Nikon\C224-IF2\Raw','COPY',false)
% nikon_rename('K:\Nikon\C224_fixed2\20200205_151050_114','K:\Nikon\C224-IF1\Raw','COPY',false)


%% Calculate Bias live
clear p
p.biaspath='K:\Nikon\C224-live\Bias';  % Output path for bias
p.imagepath='K:\Nikon\C224-live\Raw';  % Image directory. If nikon, single folder for all sites
...
%% Paths
s.experiment_name='C224-live';
s.image_drive = 'K:\Nikon\C224-live\';
...
%% Calculate Bias IF1 (using also for IF2)
clear s
s.biaspath='K:\Nikon\C224-IF1\Bias';
s.imagepath='K:\Nikon\C224-IF1\Raw';
s.shadingpath='';
s.nucradius=24;%12;
s.names={
    'DAPI';
...
s.foreground_calc = 1; % 1 for foreground calc
s.blocknum= 11;
s.prctilethresh = 50;
s.compress = .25;
...
% biasCalc(s,0);
  ...
  %% Prep processing
%%% Paths
settings.data_path = 'F:\Data\C-Cdt1\C224-live\Data\';
settings.IF_imagesessions = {'H:\8TB6\Data\Nikon\C224-IF1\', 'H:\8TB6\Data\Nikon\C224-IF2\'};
settings.live_imagepath = 'H:\8TB6\Data\Nikon\C224-live\Raw';
settings.bgcmospath = '';
settings.crop_save = 'I:\C224-IFcrop\';
settings.mask_save = 'I:\C224-IFcrop\';           % Output mask, leave empty if no save
...
settings.localbg = {[1 1 1], [1 1 1]};
settings.minringsize = 100;
settings.bgcmoscorrection = 1; %1 = correct for shading; 0 = dont correct for shading;
settings.bgsubmethod = {{'global nuclear','global cyto', 'global nuclear'},{'global nuclear', 'global cyto', 'global nuclear'}}; %'global nuclear','global cyto','tophat','semi-local nuclear'
...
Timelapse_addIF(settings,7,6,4,0)
...
%% Run Fixed
settings.IF_imagesessions = {'K:\Nikon\C224-IF1\', 'K:\Nikon\C224-IF2\'};
settings.crop_save = 'G:\8TB6\Data\Nikon\C224-IFcrop\';
settings.mask_save = 'G:\8TB6\Data\Nikon\C224-IFcrop\'; 
settings.maskname = 'mask';

% Fixed_IF(settings,2,1,5,0)
...
%% Run Fixed (FP imaged wells)

settings.data_path = 'F:\Data\C-Cdt1\C224-live\Data_FP\';
settings.IF_imagesessions = {'K:\Nikon\C224-IF1\', 'K:\Nikon\C224-IF2\',  'K:\Nikon\C224-FP\'};
settings.bgcmospath = '';
settings.crop_save = 'G:\8TB6\Data\Nikon\C224-IFcrop\';
settings.mask_save = 'G:\8TB6\Data\Nikon\C224-IFcrop\';           % Output mask, leave empty if no save
...
settings.maxjit = 500;
settings.signals = {{'DAPI_','YFP_','FarRed_'},{'DAPI_','YFP_','FarRed_'}, {'CFP_','YFP_','RFP_'}};

%%% Quantification parameters
settings.bias = {[1 1 1], [1 1 1], [1 1 1]};
settings.biasall = {[1 1 1], [1 1 1], [0 0 0]};
settings.sigblur = {[0 0 0], [0 0 0], [0 0 0]};
settings.signal_foreground = {[0 0 0], [0 0 0], [0 0 0]};
settings.bleedthrough = {[0 0 0], [0 0 0], [0 0 0]};
settings.bleedthroughslope = {};
settings.bleedthroughoff = {};
settings.ringcalc = {[1 1 1], [0 1 1], [0 0 0]};
settings.ringthresh = {[0 0 0 ], [0 0 0], [0 0 0]};
settings.punctacalc = {[0 0 0], [0 0 0], [0 0 0]};
settings.punctaThresh = {{[],[],[]},{}};
settings.punctatopsize = 2;
settings.localbg = {[1 1 1], [1 1 1], [1 1 1]};
settings.minringsize = 100;
settings.bgcmoscorrection = 1; %1 = correct for shading; 0 = dont correct for shading;
settings.bgsubmethod = {{'global nuclear','global cyto', 'global nuclear'},{'global nuclear', 'global cyto', 'global nuclear'}...
    ,{'global nuclear', 'global nuclear', 'global nuclear'}}; %'global nuclear','global cyto','tophat','semi-local nuclear'
settings.bgperctile = {[25 25 25], [25 25 25], [25 25 25]};
settings.frameIF=1;
