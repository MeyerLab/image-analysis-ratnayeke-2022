...
setting.saveName = 'C224_data.mat';
setting.dataDir = 'F:\Data\C-Cdt1\Figures\Paper\ND-Cdt1-starve\C224';
    ...
    if ~isempty(setting.crl)
        ...
        if setting.poiCrl
            S(i).POI(:,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea,'traceStats', S(i).traceStats,'nuc',S(i).crlNuc, 'cyto', S(i).crlCyto), ...
                ...
                'firstD',0,'secD',-.005,'decrease', .05*3, 'minSlope', -.06, 'early', 25,'falseCall', .9, 'cytoCheck', 0,'minExp',100,...
                'trunk',.4,'debug',0));
