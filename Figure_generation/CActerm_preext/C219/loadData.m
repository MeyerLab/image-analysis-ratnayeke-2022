...
setting.saveName = 'C219_data.mat';
setting.dataDir = 'F:\Data\C-Cdt1\Figures\Paper\CActerm_preext\C219';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).DAPI1 = S(i).IFdata(:, find(ismember(IFnames,'1_DAPI__mean')));
        S(i).YFP1 = S(i).IFdata(:, find(ismember(IFnames,'1_YFP__mean')));
        S(i).FarRed1 = S(i).IFdata(:, find(ismember(IFnames,'1_FarRed__mean')));
    ...
    if ~isempty(setting.apc)
        ...
        if setting.quiescent
            ...
            buff = 20;
            postMitosis = 1;
            [badTracesApc] = gateSigTraces(traceGem, S(i).traceStats, buff, postMitosis, minMaxThresh, minMaxNoise,0);
        ...
        if setting.poiApc
            if setting.quiescent
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    struct('postBuffer',10,'smooth',7,'cycling',0,...
                    'buff',1,'thresh',.3,'preBuffer',1,'lowThresh',20, 'increase',10*.3,'trunc',50, 'medfilt',0,'early', 20));
            else
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    struct('postBuffer',10,'smooth',7,'cycling',1,...
                    'buff',10,'thresh',.4,'preBuffer',25,'lowThresh',20, 'increase',10*.4,'trunc',100, 'medfilt',0,'early', 15));
            end
            %S(i) = gateout_all(S(i),~(badTracesAPC));
...
%         S(i).crlNuc = fillTraceVals(S(i).crlNuc, S(i).traceStats, 5);
        ...
        if setting.poiCrl
            S(i).POI(:,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea,'traceStats', S(i).traceStats,'nuc',S(i).crlNuc, 'cyto', S(i).crlCyto), ...
                struct('cycling',~setting.quiescent,'smooth',5 ,'buff',10 , 'postBuffer', 4 ,'postBufferSec',2,...
                'firstD',0,'secD',-.005,'decrease', .02*4, 'minSlope', -.02, 'early', 5,'falseCall', .9, 'cytoCheck', 100,'minExp',100,...
                'debug',0));
             
%             S(i).POI(:,3) = findCRL4Act(S(i).crlAreaNorm, S(i).traceStats, ...
%                 struct('cycling',~setting.quiescent,'low',.1,'smooth',7, 'smoothSec',5,'postBufferSec',1,...
%                 'firstD',0,'secD',-.005,'early', 30,'falseCall', .8,'buff',0, 'postBuffer', 4,'decrease', .02*4, 'debug',1));
    ...
    S(i).allDAPI1 = allIFdata(:, find(ismember(IFnames,'1_DAPI__mean')));
    S(i).allDNA = S(i).allDAPI1 .* S(i).allIFarea;
    S(i).allYFP1 = allIFdata(:, find(ismember(IFnames,'1_YFP__mean')));
    S(i).allFarRed1 = allIFdata(:, find(ismember(IFnames,'1_FarRed__mean')));
