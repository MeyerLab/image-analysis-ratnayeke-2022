...
setting.saveName = 'C210_data.mat';
setting.dataDir = 'F:\Data\C-Cdt1\Figures\Paper\CActerm_preext\C210_longterm';
        ...
        if setting.poiApc
            if setting.quiescent
                ...
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcAreaNormM, S(i).traceStats, ...
                    ...
                    'buff',3,'thresh',.001,'preBuffer',5,'lowThresh',.05, 'increase',.01,'trunc',.5, 'medfilt',0,'early', 10,'debug',0));
            end
            %S(i) = gateout_all(S(i),~(badTracesAPC));
...
%         S(i).crlNuc = fillTraceVals(S(i).crlNuc, S(i).traceStats, 5);
        ...
        if setting.poiCrl
            S(i).POI(:,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea,'traceStats', S(i).traceStats,'nuc',S(i).crlNuc, 'cyto', S(i).crlCyto), ...
                struct('cycling',~setting.quiescent,'smooth',7 ,'buff',10 , 'postBuffer', 3 ,'postBufferSec',2,...
                'firstD',0,'secD',-.005,'decrease', .04*3, 'minSlope', -.02, 'early', 15,'falseCall', .9, 'cytoCheck', 150,'minExp',100,...
                'trunk',.4,'debug',0));
            ...
            if setting.poiG2
                [S(i).POI(:,5), badTracesCRL] = findCRLInact(S(i).crlNormAct, S(i).traceStats, S(i).POI(:,4), ...
                    struct('postBuffer',10,'smooth',9,...
                    'buff',10,'thresh',.0025,'preBuffer',5,'lowThresh',.2, 'increase',.02,'trunc',.5, 'medfilt',0,'early', 20,'debug',0));
            end
