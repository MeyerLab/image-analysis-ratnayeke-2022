% addpath(genpath('/media/meyer/SSD 1TB/cell-cycle-tracking/Tracking_dependencies'));
%% Rename live sequence
% nikon_rename_time('K:\Nikon\C221-live\Raw2','K:\Nikon\C221-live\Raw','COPY',false)

%% Calculate Bias live
clear p
p.biaspath='K:\Nikon\C221-live\Bias';  % Output path for bias
p.imagepath='K:\Nikon\C221-live\Raw';  % Image directory. If nikon, single folder for all sites
...
%% Paths
s.experiment_name='C221-live';
s.image_drive = '/media/meyer/SSD 1TB/Nikon/C221-live/';
s.savepath=fullfile('/media/meyer/SSD 1TB/',s.experiment_name,'Data');
...
s.maskname = 'nucedge_';
s.register = 0;
s.register_exception = [110];

%%% Quantification parameters
s.bgcmoscorrection = 1;
s.bias = [1 1 ];
s.signal_foreground = [0 0];
s.bgsubmethod = {'global nuclear','global nuclear'}; %Options:'global nuclear','global cyto','tophat','semi-local nuclear', 'none'
s.compression = 4;
s.bgprctile = [25 25];
s.sigblur = [3 3];
s.localbgmeasure = [0 0];
s.ringcalc = [0 1];
s.ringthresh = [0 0];
s.punctacalc = 0;
...
%% Calculate Bias IF1
clear s
s.biaspath='/media/meyer/SSD 1TB/Nikon/C221-IF1/Bias';
s.imagepath='/media/meyer/SSD 1TB/Nikon/C221-IF1/Raw';
s.shadingpath='';
s.nucradius=24;%12;
s.names={
    'DAPI';
...
% biasCalc(s,0);
  ...
  %% Prep processing timelapse add IF
%%% Paths
settings.data_path = '/media/meyer/SSD 1TB/C221-live/Data/';
settings.IF_imagesessions = {'/media/meyer/SSD 1TB/Nikon/C221-IF1/'};
settings.live_imagepath = '/media/meyer/SSD 1TB/Nikon/C221-live/Raw';
settings.bgcmospath = '';
settings.crop_save = '';
settings.mask_save = '/media/meyer/SSD 1TB/Nikon/C221-IF1/';           % Output mask, leave empty if no save
...
% Timelapse_addIF(settings,6,7,5,1)


%% Run timelapse add IF
rows = [5 6 7];
cols = [2:11];
sites = [1:9];
