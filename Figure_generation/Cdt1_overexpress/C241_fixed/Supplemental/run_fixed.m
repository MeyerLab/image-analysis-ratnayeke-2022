cd('F:\Data\C-Cdt1\C241-fixed')
%% Rename IF 
nikon_rename('O:\C241-IF\20220323_142602_453',...
     'O:\C241-IF\Raw','COPY',false,'SINGLE_SITE',false)


%% Calculate Bias IF
clear p
p.biaspath='O:\C241-IF\Bias';  % Output path for bias
p.imagepath='O:\C241-IF\Raw';  % Image directory. If nikon, single folder for all sites
...
%% Setup IF images
clear settings
settings.data_path = 'F:\Data\C-Cdt1\C241-fixed\Data\';      % Output location of mat files
settings.IF_imagesessions = {'O:\C241-IF\'};    
...
settings.signals = {{'DAPI','CFP','YFP','RFP','FarRed'}};  % Each imaging session in a cell inside cell array
...
settings.bgsubmethod = {{'global nuclear','global nuclear', 'global nuclear',  'none', 'none'}}; % 'global nuclear','global cyto','tophat','semi-local nuclear'
