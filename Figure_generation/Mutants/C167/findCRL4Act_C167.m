function [ POI ] = findCRL4Act_C163( CRLtrace, traceStats, settings )
...
%         tracePost = tracePost/max(tracePost); % renormalize
        ...
        if settings.smooth > 1
            smoothTrace = nansmooth(tracePost, settings.smooth);
        else
            smoothTrace = tracePost;
        end
        firstD = diff(smoothTrace);
        ...
        if length(smoothTrace) > settings.postBuffer & settings.postBuffer > 0
            for j = 1:numFrames - settings.postBuffer
                ...
                allSlope(j) = mean(firstD(j:j+settings.postBuffer)) < settings.firstD;
                gate(j) =  maxFutureHeight(j) & futureHeightDec(j) & allSlope(j);
            end
