function [S] = loadData_C167(conditions, dataDir)
...
setting.saveDir = 'F:\Data\C-Cdt1\Figures\Paper\mutants\C167\';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).YFP1(S(i).YFP1 < .01) = .01;
        S(i).FarRed1 = S(i).IFdata(:, find(ismember(IFnames,'1_FarRed_mean')));
        S(i).FarRed1(S(i).FarRed1 < .01) = .01;
        S(i).DAPI2 = S(i).IFdata(:, find(ismember(IFnames,'2_DAPI_mean')));
        S(i).DAPI2(S(i).DAPI2 < .01) = .01;
        S(i).YFP2 = S(i).IFdata(:, find(ismember(IFnames,'2_YFP_mean')));
        S(i).YFP2(S(i).YFP2 < .01) = .01;
        S(i).FarRed2 = S(i).IFdata(:, find(ismember(IFnames,'2_FarRed_mean')));
        S(i).FarRed2(S(i).FarRed2 < .01) = .01;
    ...
    if ~isempty(setting.apc)
        ...
        if setting.quiescentAnalysis
            ...
            S(i) = gateout_all(S(i),~(highNoise | highMean));
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact_C167(S(i).apcNuc, S(i).traceStats, ...
...
%         S(i).crlNuc = fillTraceVals(S(i).crlNuc, S(i).traceStats, 5);
        ...
        S(i) = gateout_all(S(i),~( highNoise | lowMax | lowTracestart));
        ...
        if setting.poiCrl
            S(i).POI(:,3) = findCRL4Act_C167(S(i).crlAreaNorm,S(i).traceStats, ...
                struct('cycling',~setting.quiescentAnalysis,'low',.1,'smooth',5, ...
                'firstD',-.02,'secD',-.005,'early', 30,'falseCall', .8,'buff',0, 'postBuffer', 3,'decrease', .02*3));
