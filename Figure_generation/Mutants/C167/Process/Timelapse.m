...
%%% Paths
s.experiment_name='C167-live';
s.image_drive = 'I:\4TB9\Data\';
...
s.bgcmospath='F:\MATLAB\BGimages\IX_Liu\cmosoffset_bin1.mat';
...
if ~exist([s.savepath,'tracedata_',shot,'.mat'],'file')
    ...
    for i = firstgoodindex:totalframes
        ...
        for j = 1:length(s.signals)
            ...
            %%% Puncta analysis
            if j == s.punctacalc
                p = punctaModule(unblurred{j}, nuc_label, nanvec, s.punctaThresh, s.varThresh);
...
function p = punctaModule(punctaSig, nuc_label, nanvec, punctaThresh, varThresh)
p = createPunctaStruct();
...
p.varMean = nanvec;
p.varStd = nanvec;
for t = 1:length(s.punctaThresh)
    p.filtMaskedMean{t} = nanvec;
    p.filtMaskedArea{t} = nanvec;
end
for t = 1:length(s.varThresh)
