...
setting.saveDir = 'F:\Data\C-Cdt1\Figures\Paper\Replication factors\C194\';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).FarRed1 = S(i).IFdata(:, find(ismember(IFnames,'1_Cy5_mean')));
        S(i).FarRed1cyt = S(i).IFdata(:, find(ismember(IFnames,'1_Cy5_cyto ring')));
        S(i).YFP1 = S(i).IFdata(:, find(ismember(IFnames,'1_YFP_mean')));
        S(i).YFP1cyt = S(i).IFdata(:, find(ismember(IFnames,'1_YFP_cyto ring')));

        
        S(i).CFP2 = S(i).IFdata(:, find(ismember(IFnames,'2_CFP_mean')));
        S(i).YFP2 = S(i).IFdata(:, find(ismember(IFnames,'2_YFP_mean')));
        S(i).YFP2cyt = S(i).IFdata(:, find(ismember(IFnames,'2_YFP_cyto ring')));

        
        S(i).CFP3 = S(i).IFdata(:, find(ismember(IFnames,'3_CFP_mean')));
        S(i).YFP3 = S(i).IFdata(:, find(ismember(IFnames,'3_YFP_mean')));
        S(i).YFP3cyt = S(i).IFdata(:, find(ismember(IFnames,'3_YFP_cyto ring')));
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    ...
                    'buff',1,'thresh',.7,'preBuffer',1,'lowThresh',20, 'increase',5*1,'trunc',100, 'medfilt',0,'early', 30));
            else
