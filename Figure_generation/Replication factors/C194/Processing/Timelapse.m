...
%%% Paths
s.experiment_name='C194-live';
s.image_drive = 'E:\Nikon';
s.savepath=fullfile('F:\Data\C-Cdt1\',s.experiment_name,'Data');
s.imagepath=fullfile(s.image_drive,s.experiment_name,'Raw'); %single directory
s.biaspath=fullfile(s.image_drive,s.experiment_name,'Bias');
s.maskpath=fullfile(s.image_drive,s.experiment_name,'Mask');
s.bgcmospath='';
s.formatCode = 'Time%1$05d_Well%2$s_Point%2$s_%3$04d_*.tiff';

%%% General parameters
s.startFrame=1;   %Frame to start analyzing from
s.endFrame=87;  %Frame to stop analyzing
...
if ~exist(fullfile(s.savepath,['tracedata_',shot,'.mat']),'file')
    ...
    [firstgoodindex,blurthreshhigh,blurthreshlow,numthresh,badframes,height,width] = ...
        timelapsesetup_nikon(rawdir,row,col,site, s.formatCode,frames,s.nucr,blobthreshold,s.debrisarea,badframes,s.maskwrite);
    ...
    for i = firstgoodindex:totalframes
        ...
        for j = 1:length(s.signals)
            shotSearch = sprintf(s.formatCode,frames(i)-1,rowColumnTowellName(row,col),site-1);
            fileName = findFile(rawdir,shotSearch);
            raw{j} = double(imread(fullfile(rawdir,fileName),j));
