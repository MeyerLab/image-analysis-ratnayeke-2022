function [ riseTime, badTraces] = findAPCInact_C163( apcTrace, traceStats, settings )
%
...
for i=1:size(apcTrace, 1)
    ...
    if length(trace) > settings.postBuffer
        ...
        for j = 1:numFrames - settings.postBuffer
            ...
            allSlope(j) = sum(sigSlope(j+1:j+settings.postBuffer) > settings.thresh) >= settings.postBuffer-1;
...
%         filterScore = 2*sigFwdSlope - 1*signal + sigTime + 1;
        ...
        elseif (signal(end))<settings.lowThresh %change to endpoint only for C200
            riseTime(i)=NaN;
        else
            badTraces(i) = 1;
        end
