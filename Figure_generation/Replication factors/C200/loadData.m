...
setting.saveDir = 'F:\Data\C-Cdt1\Figures\Paper\Replication factors\C200\';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).FarRed3 = S(i).IFdata(:, find(ismember(IFnames,'3_FarRed_mean')));
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    ...
                    'buff',1,'thresh',1,'preBuffer',1,'lowThresh',10, 'increase',5*1,'trunc',50, 'medfilt',0,'early', 40,'debug',0));
            else
