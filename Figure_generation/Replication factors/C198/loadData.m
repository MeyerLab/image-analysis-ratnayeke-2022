...
setting.saveDir = 'F:\Data\C-Cdt1\Figures\Paper\Replication factors\C198\';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).FarRed1cyt = S(i).IFdata(:, find(ismember(IFnames,'1_FarRed__cyto ring')));

        S(i).YFP1 = S(i).IFdata(:, find(ismember(IFnames,'1_YFP__mean')));
        
        S(i).FarRed2 = S(i).IFdata(:, find(ismember(IFnames,'2_FarRed__mean')));
        S(i).YFP2 = S(i).IFdata(:, find(ismember(IFnames,'2_YFP__mean')));
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    ...
                    'buff',1,'thresh',.7,'preBuffer',1,'lowThresh',10, 'increase',5*.6,'trunc',50, 'medfilt',0,'early', 2));
            else
