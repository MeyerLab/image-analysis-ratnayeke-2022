...
if ~exist(fullfile(s.savepath,['tracedata_',shot,'.mat']),'file')
    ...
    ind = max([1, find(s.startFrame <= s.Frames,1,'last') - 1]);

    switch s.microscope
        case 'IXM'
            rawdir = fullfile(s.imagepath{ind},shot,[shot '_']);
        case 'nikon'
            rawdir = fullfile(s.imagepath{ind});
    end
    ...
    frames = s.startFrame:s.Frames(end);
    totalframes = numel(frames);
    badframes = ones(s.Frames(end),1)*NaN;
    if s.startFrame>1
        badframes(1:s.startFrame-1) = 0;
    end
    jitters = zeros(s.Frames(end),2);
    blocksize = 10000;
    maxcellnum = blocksize;
    tracedata = ones(maxcellnum,s.Frames(end),parameternum)*NaN;
    ...
    for i = firstgoodindex:totalframes
        ...
        %Set frame index based on imaging sessions
        ind = max([find(f <= s.Frames,1,'last') - 1, 1]);
        subtractFrame = s.Frames(ind);
        
        %%% Load images %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for j = 1:length(s.signals)
            switch s.microscope
                case 'IXM'
                    rawdir = fullfile(s.imagepath{ind},shot);
                    fileName = [shot '_' s.signals{j},num2str(f-subtractFrame+1),'.tif'];
                    raw{j} = single(imread(fullfile(rawdir,fileName)));
                case 'nikon'
                    rawdir = fullfile(s.imagepath{ind});
                    shotSearch = sprintf(s.formatCode,f-subtractFrame,rowColumnTowellName(row,col),site-1);
...
%         
        ...
        if maxcellnum-max(cellid)<blocksize
            tempdata = ones(blocksize,s.Frames(end),parameternum)*NaN;
