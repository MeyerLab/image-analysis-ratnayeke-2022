function Timelapse_C124(row,col,site,debug_mode)
% row = 2;
% col = 2;
% site = 1;
% debug_mode = 1;
...
%%% Paths
experiment_name='C124-live';
savepath=['F:\Data\C-Cdt1\',experiment_name,'\Data\'];
imagepath=['I:\4TB1\Data\',experiment_name,'\Raw\'];
biaspath=['I:\4TB1\Data\',experiment_name,'\Bias\'];
maskpath=['I:\4TB1\Data\',experiment_name,'\Mask\'];
bgcmospath='F:\MATLAB\BGimages\IX_Liu\cmosoffset_bin1.mat';

%%% General parameters
startFrame=30;   %Frame to start analyzing from
endFrame=93;  %Frame to stop analyzing
...
bgcmoscorrection = 1;
bias = [1 1 1];
signal_foreground = [0 1 1];
bgsubmethod = {'global nuclear','global nuclear','global nuclear'}; %'global nuclear','global cyto','tophat','semi-local nuclear'
compression = 4;
sigblur = [3 3 3];
localbgmeasure = [0 1 1];
ringcalc = [0 0 0];
ringthresh = [0 100 100];
punctacalc = 0;
...
if ~exist([savepath,'tracedata_',shot,'.mat'],'file')
    ...
    for i = firstgoodindex:totalframes
        ...
        tempframe = zeros(height,width,3);
        tempframe(:,:,1) = imadjust(mat2gray((raw{3})));
        tempframe(:,:,2) = extractmask;;
        %tempframe(:,:,3) = marker_mask;
        figure,imshow(tempframe);
