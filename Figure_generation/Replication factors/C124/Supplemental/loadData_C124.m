function [S] = loadData_C124(conditions, dataDir)
...
for i = 1:uniqueCondnum
    ...
    if ~isempty(setting.apc)
        ...
        if setting.quiescentAnalysis
            ...
            S(i) = gateout_all(S(i),~(highTracestart | highMean | highNoise ));
        else
            % Cycling
            minMaxThresh = [10 100];
            minMaxNoise = [-500 500];
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact_C124(S(i).apcNuc, S(i).traceStats, ...
                    struct('postBuffer',5,'smooth',5,'cycling',0,...
                    'buff',1,'thresh',.75,'preBuffer',1,'lowThresh',20, 'increase',5*.75,'trunc',50, 'medfilt',0,'early', 30));
            else
