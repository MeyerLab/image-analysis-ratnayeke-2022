function Timelapse_upgrade(s,row,col,site,debug_mode,varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%% Paths
% s.experiment_name='D129-live';                % Experiment name
% s.image_drive = 'G:\D129-live\';    % Location of images base folder
% s.savepath=fullfile('F:\Data\D-Replication Initiation\',s.experiment_name,'Data'); % Output location of mat files
...
% s.register_exception = [81 82 83 225 226 227 353 354 355 466 467 468];        % If don't register images, manually put in sites to register
% 
% %%% Quantification parameters
% s.bgcmoscorrection = 1;
% s.bias = [0 0 0 0];
% s.signal_foreground = [0 0 0 0];
% s.nuc_dilate = [2 1 1 1]; %1 for nuclear signals 2 for cytoplasmic 
...
% s.firstsegmethod = 'concavity';  % Options: 'concavity', 'log', 'single adapt', 'double adapt', 'single', 'double', 'log contour'
% s.secondsegmethod = 'concavity'; % Options: 'concavity', 'log', 'single adapt', 'double adapt', 'single', 'double','log contour'
...
if ~exist(fullfile(s.savepath,['tracedata_',shot,'.mat']),'file')
    ...
    bg = zeros(length(s.signals),s.endFrame);
    ...
    trackparams = {s.nucr,s.maxjump,s.debrisarea,s.masschangethreshold,s.areachangethreshold,s.daughtervariance, s.massweight, s.adaptlink, s.minjump, s.masschangeLAP};
    ...
    for i = 1:length(s.signals)
        if s.bias(i)
            load(fullfile(s.biaspath,[s.signals{i},'_',num2str(site),'.mat']),'bias');
            bias_cell{i} = single(bias);
            clear bias
        end
    end
...
%         gate = isLarge & ~isMitotic & ~isDim;
        ...
        clear label_mask antiborder_mask
        %% background subtract: masked %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        for j = 1:length(s.signals)
            if s.signal_foreground(j)
                mask_total  =  whole_mask | foreground{j};
                nanmask{j} = imdilate(mask_total,strel('disk',s.nucr*s.nuc_dilate(j)));
            else
                nanmask{j} = imdilate(whole_mask,strel('disk',s.nucr*s.nuc_dilate(j)));
            end
            ...
            switch s.bgsubmethod{j}
                case 'global mode'
                    bg(j,f) = measure_global_background(blurIm,nanmask{j},s.compression,[],'mode');
                    realIm{j} = blurIm - bg(j,f);                  
                    if s.punctacalc == j
                        bg_temp = measure_global_background(blurIm,nanmask{j},s.compression,[],'mode');
                        unblurred{j} = raw{j} - bg_temp;
                    end
                case 'global percentile'
                    bg(j,f) = measure_global_background(blurIm,nanmask{j},s.compression,s.bgprctile(j),'percentile');
                    realIm{j} = blurIm - bg(j,f);
                    if s.punctacalc == j
                        bg_temp = measure_global_background(blurIm,nanmask{j},s.compression,s.bgprctile(j),'percentile');
                        unblurred{j} = raw{j} - bg_temp;
                    end
                case 'tophat'
                    realIm{j} = imtophat(blurIm,strel('disk',s.nucr,0));
                    if s.punctacalc == j                       
                        unblurred{j} = imtophat(raw{j},strel('disk',s.nucr,0));
                    end
                case 'semi-local nuclear'
                    realIm{j} = bgsubmasked_global_2(blurIm,nanmask{j},11,s.compression,s.bgprctile(j));
                    if s.punctacalc == j
        ...
        %%% Clear unnecessary variables
        clear raw mask_total foreground mask_total blurIm label_mask bounds whole_mask
        
        %% Extract features %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        [nuc_label,numcells] = bwlabel(nuc_mask);
        nuc_info = regionprops(nuc_mask,realIm{s.nucChannel},'Area','Centroid','MeanIntensity');
        ...
        if i>firstgoodindex
            ...
            [tracedata,curdata,tracking,nuc_label, mitosis_flag, merge_flag] = ...
                adaptivetrack_LAP(f,lastgoodframe,f,tracedata,curdata,tracking,realIm{s.nucChannel},nuc_label,jitters(f,:),trackparams,debugpackage);
%                   [tracedata,curdata,tracking,nuc_label, mitosis_flag] = ...
%                 adaptivetrack_NR_out(f,lastgoodframe,f,tracedata,curdata,tracking,realIm{s.nucChannel},nuc_label,jitters(f,:),trackparams,debugpackage);
            badframes(f) = 0;
            ...
            %{           
            prev_outline = imdilate(bwmorph(prev_label > 0,'remove'),strel('disk',2));
            nuc_outline = imdilate(bwmorph(nuc_label > 0,'remove'),strel('disk',2));
            tempframe=zeros(height,width,3);
            tempframe(:,:,1)=prev_outline;
            tempframe(:,:,2)=nuc_outline;
            figure,imshow(tempframe);
            hold on
            track_x = [tracedata(:,lastgoodframe,1),  curdata(:,1)] - jitters([lastgoodframe f],1)';
            track_y = [tracedata(:,lastgoodframe,2),  curdata(:,2)] - jitters([lastgoodframe f],2)';
            mitoses_x = track_x(mitosis_flag,1);
            mitoses_y = track_y(mitosis_flag,1);
            inds = ~any(isnan(track_x),2);
            track_x = track_x(inds,:);
            track_y = track_y(inds,:);
            plot(track_x',track_y','w','LineWidth',2)
            scatter(mitoses_x,mitoses_y,100,'c','LineWidth',1.5)
            %}
        end
        ...
        clear border_mask border_label
        %% Measure features%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if any(s.ringcalc)
            ...
            ring_label = getcytoring_thicken(all_label,innerrad,outerrad,realIm{2});
            ...
            tempframe = zeros(height,width,3);
            tempframe(:,:,1) = imadjust(mat2gray(realIm{1}));
            tempframe(:,:,2) = extractmask;;
            %tempframe(:,:,3) = marker_mask;
            figure,imshow(tempframe);
            %}
    ...
    %%% save data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    save(fullfile(s.savepath,['tracedata_',shot,'.mat']),'tracedata','genealogy','jitters','bg','-v7.3');
...
erode_nuc = imerode(nuclear_mask, strel('disk',5,0));
