...
%%% Conditions %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.conditions = {
%             '45',2:3,2:4,1,[0 0 0],[0 10]; %1    
% 
        'Cdt1',2:3,5:7,1:9,[0 0 0],[0 10]; %1    
        'Cdt1-delPIP',4:5,5:7,1:9,[0 0 0],[0 10]; %1
        'Cdt1-delCy',6:7,5:7,1:9,[0 0 0],[0 10]; %1 
        };

%%% Paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.dataDir = 'F:\Data\C-Cdt1\C230-live2\Data';
...
settings.saveName = 'C230_data.mat';
settings.saveDir = 'C:\Users\nalin\Documents\GitHub\image-analysis-ratnayeke-2022\Figure_generation\Cdt1_mutant_deg\C230\Supplemental';
%%% Analysis Options %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(S)
    ...
    if strcmp(settings.type,'release')
        S(i) = gateout_all(S(i),~S(i).isDaughter);
    end
...
%             inds = find(highNoise);
    ...
    if ~isempty(settings.sig)
        bg_ind = find(ismember(names,[settings.sig '_mean']))- 4;
        S(i).sig_bg_median = median(S(i).bg(:,:,bg_ind),1);
        S(i).sigNuc = S(i).traceData(:,:,ismember(names,[settings.sig '_mean']))+ S(i).bg(:,:,bg_ind);
        S(i).sig_bg_perc = prctile(S(i).sigNuc,5,1);
        
        S(i).sigNuc = S(i).sigNuc - S(i).sig_bg_median;
...
%         meanWindow = 1:400;
        ...
        lowMean = nanmean(S(i).pcnaNuc(:,1:end),2) < lowThresh;
        ...
        if settings.poiPCNA
            for j = 1:size(S(i).filterArea,3)
                S(i).filterPOI(:,j) = getPCNAstart(S(i).filterArea(:,:,j),S(i).traceStats,...
                    struct('initoffset', 10,'thresh', 40, 'lowthresh',3,'debug',0));
            end
