function sensor = loadData_A40(conditions, pth)
...
for ii = 1:uniquecondnum
    ...
    sensor(ii).DAPI = IFdata_combined(:, find(ismember(names,'1_DAPI_mean')));
    sensor(ii).YFP = IFdata_combined(:, find(ismember(names,'1_YFP_mean')));
    sensor(ii).YFPcyt = IFdata_combined(:, find(ismember(names,'1_YFP_cyto ring')));
    sensor(ii).FarRed = IFdata_combined(:, find(ismember(names,'1_FarRed_mean')));
...
for i = 1:condnum
    ...
    sensor(i).dna = (sensor(i).area.*sensor(i).DAPI);
    %ind = sensor(i).area > 200 & sensor(i).area < 500;
    ind = sensor(i).area > 200;
