...
% settings.crop_save = 'H:\8TB4\C195-total-IFcrop\';
...
maskdir = fullfile(settings.maskpath,shot);
if ~exist(maskdir,'dir') && ~isempty(settings.maskpath)
    mkdir(maskdir);
end
...
for i = 1:length(names)
    ...
    for j = 1:length(names{i})
        switch settings.microscope
            ...
            case 'nikon'
                shotSearch = sprintf(settings.formatCode,frame-1,rowColumnTowellName(row,col),site-1);
...
%%% Save mask
if ~isempty(settings.maskpath)
    extractmask = bwmorph(nuc_mask_aligned,'remove');
    imwrite(uint16(extractmask),fullfile(maskdir, [settings.maskname shot '.tif']));
end
%% MEASSURE FEATURES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
