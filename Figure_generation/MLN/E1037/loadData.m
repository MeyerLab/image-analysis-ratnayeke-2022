...
setting.saveName = 'C1037_data.mat';
setting.dataDir = dataDir;
% setting.dataDir = 'F:\Data\C-Cdt1\Figures\Paper\GMNN_dynamics\C179-HeLa';
...
for i = 1:uniqueCondnum
    ...
    if setting.IFoption
        ...
        S(i).CFP1 = S(i).IFdata(:, find(ismember(IFnames,'1_CFP_mean')));
        S(i).RFP1 = S(i).IFdata(:, find(ismember(IFnames,'1_RFP_mean')));
...
%                 S(i).filterPOI(:,j) = getPCNAstart(S(i).filterArea(:,:,2),S(i).traceStats,...
%                     struct('initoffset', 4,'thresh', 50, 'lowthresh',5,'debug',1));
                [S(i).filterPOI(:,j), badTracesPCNA] = getPCNArise(S(i).filterArea(:,:,j), S(i).traceStats, ...
                    struct('postBuffer',2,'smooth',0,'cycling',1,...
                    'buff',0,'thresh',5,'preBuffer',6,'lowThresh',10, 'increase',20,'trunc',200, 'medfilt',0,'early', 5,'debug',0));
    ...
    if setting.IFoption        
        ...
        S(i).allCFP1 = allIFdata(:, find(ismember(IFnames,'1_CFP_mean')));
        S(i).allRFP1 = allIFdata(:, find(ismember(IFnames,'1_RFP_mean')));
