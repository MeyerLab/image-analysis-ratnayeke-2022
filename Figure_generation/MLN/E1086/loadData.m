...
setting.quiescent = 1;   %0:cycling cells 1:serum starved cells
...
setting.saveName = 'E1086_data.mat';
setting.dataDir = dataDir;
...
%         S(i) = gateout_all(S(i),~(badTracesAPC));
        ...
        if setting.poiApc
            if setting.quiescent
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    struct('postBuffer',8,'smooth',9,'cycling',0,...
                    'buff',1,'thresh',2,'preBuffer',6,'lowThresh',40, 'increase',8*2,'trunc',200, 'medfilt',0,'early', 40,'debug',0));
            else
    ...
    if setting.IFoption        
        ...
        S(i).allDAPI = allIFdata(:, find(ismember(IFnames,'DAPI_mean')));
        S(i).allDNA = S(i).allDAPI .* S(i).allIFarea;
        S(i).allRFP = allIFdata(:, find(ismember(IFnames,'RFP_mean')));    
        S(i).allFarRed = allIFdata(:, find(ismember(IFnames,'FarRed_mean')));
    end
end
