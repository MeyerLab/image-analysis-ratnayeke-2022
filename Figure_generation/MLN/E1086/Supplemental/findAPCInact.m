...
for i=1:size(apcTrace, 1)
    ...
    if length(trace) > settings.postBuffer
        ...
        prevSlope = zeros(1,numFrames);
        early = zeros(1,numFrames);
        sigSlope = gradient(signal);
        for j = settings.preBuffer+1:numFrames - settings.postBuffer
...
%             allSlope(j) = sum(sigSlope(j:j+settings.postBuffer) > settings.thresh) >= settings.postBuffer;
            allSlope(j) = sum(sigSlope(j+1:j+settings.postBuffer) > settings.thresh) >= settings.postBuffer- round(settings.postBuffer * .1);
            prevSlope(j) = (mean((sigSlope(j-settings.preBuffer:j-1)))) < settings.thresh;
            if settings.cycling
        ...
        gate = presentHeight & minFutureHeight & futureHeightInc & allSlope & early; %& prevSlope;
