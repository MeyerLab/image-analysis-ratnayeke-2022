function Timelapse_3i(site,row,col,debug_mode)
% site = 1;
% row = 2;
% col = 3;
% debug_mode = 0;
...
%%% Paths
s.experiment_name='C173-3i-live';
s.savepath=['F:\Data\C-Cdt1\',s.experiment_name,'\Data\'];
s.imagepath=['F:\Data\3i Data\PCNA 45\C173-3i-ind\Raw\'];
s.maskpath=['F:\Data\3i Data\PCNA 45\C173-3i-ind\Mask\'];

%%% General parameters
s.startFrame=1;   %Frame to start analyzing from
s.endFrame=81;  %Frame to stop analyzing
s.signals={'CFP_','YFP_','RFP_'};
s.magnification=60; %10=10x or 20=20x
...
s.punctacalc = 2;
s.punctaThresh = [100:5:150];
s.punctaCountThresh = [14:2:30];
s.varThresh = [30:5:50];
...
s.nucr = 48; %10x bin1 = 12 and 20x bin2 = 12
s.blurradius  =  10; %10x: 3
s.soliditythresh = 0.8;
s.debrisarea = 1000; % 10x = 100 % 150 if no mitosis
s.boulderarea = 13000; % 10x = 1500
blobthreshold = -0.02;

%%% Tracking parameters
s.maxjump = s.nucr*0.25;
...
if ~exist([s.savepath,'tracedata_',shot,'.mat'],'file')
    ...
    if s.punctacalc
        parameternum = parameternum + 4 + 2*length(s.punctaThresh) + 2*length(s.varThresh) + length(s.punctaCountThresh);
    end
    ...
    for i = firstgoodindex:totalframes
        ...
        if any(s.ringcalc)
            ...
            elseif s.magnification == 60 && s.binsize == 2
                innerrad = 4; outerrad = 15;
            else
        ...
        for j = 1:length(s.signals)
            ...
            %%% Puncta analysis
            if j == s.punctacalc
                p = punctaModule(unblurred{j}, nuc_label, nanvec, s.punctaThresh, s.punctaCountThresh, s.varThresh,debug_mode, numlivecells, nuc_info,cellid);
        ...
        tracedata(cellid,f,:) = ...
            ...
            cell2mat(p.varMaskedMean), cell2mat(p.varMaskedArea),cell2mat(p.punctaNum)];
    ...
    names = output_names_puncta_num(s.signals,s.localbgmeasure, s.ringcalc, s.punctacalc, s.punctaThresh, s.varThresh, s.punctaCountThresh);
...
function p = punctaModule(punctaSig, nuc_label, nanvec, punctaThresh, punctaCountThresh, varThresh, debug_mode, numlivecells, nuc_info,cellid)
p = createPunctaStruct();
...
for t = 1:length(punctaCountThresh)
    p.punctaNum{t} = nanvec;
end

%% Preprocess PCNA image
punctaBlur = imgaussfilt(punctaSig, 20); %blurred to remove nucleoli
punctaSemiBlur = imgaussfilt(punctaSig, .75);
...
nucleolus_mask = (punctaDiff < -50);
nucleolus_mask = imdilate(nucleolus_mask, strel('disk',5,0));
nucleolus_mask = bwareaopen(nucleolus_mask,50);
analysis_mask = erode_nuc & ~nucleolus_mask;

%Image transformations quantification
punctaFilt = imtophat(punctaSig, strel('disk',5,0));
punctaVar = stdfilt(punctaSig,ones(5));
...
%spot counter
b = bpass(punctaSig,1,3);
for t = 1:length(punctaCountThresh)
    pk = pkfnd(b,punctaCountThresh(t),3);
    punctaNumIm{t} = zeros(size(punctaSig));
    for punct = 1:length(pk)
        punctaNumIm{t}(pk(punct,1),pk(punct,2)) = 1;
    end
end
...
%{
      ...
      pk = pkfnd(b,25,3);
      figure, hold on
      imshow(punctaSig,[0 400])
      hold on
      scatter(pk(:,1),pk(:,2),'.r')

      imtool(punctaSig)
      imtool(punctaSemiBlur)
      imtool(punctaFilt)
...
for n = 1:numlivecells
    ...
    for t = 1:length(punctaCountThresh)
        p.punctaNum{t}(n) = sum(punctaNumIm{t}(nuc_info(cc).PixelIdxList));
    end
end
end
...
switch segMethod
    case 'concavity'
        ...
        %imblur = imfilter(image,fspecial('gaussian',10),'symmetric');
