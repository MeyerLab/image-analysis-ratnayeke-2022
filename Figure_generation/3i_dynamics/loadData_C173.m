...
setting.saveName = 'C173_data.mat';
setting.saveDir = 'F:\Data\C-Cdt1\Figures\Paper\3i_dynamics\all';
%% PROCESS DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Load data header
...
for i = 1:uniqueCondnum
    ...
    for c = condRow'
        rowMat = cell2mat(conditions(c,3));
        colMat = cell2mat(conditions(c,4));
        siteMat = cell2mat(conditions(c,2));
        for row = rowMat
            for col = colMat
                for site = siteMat
                    shot = [num2str(site),'_',num2str(row),'_',num2str(col)];
    ...
    if setting.IFoption
        ...
        S(i).FarRedHigh = S(i).IFdata(:, find(ismember(IFnames,'1_FarRedHigh_mean')));
        S(i).FarRedHighbg = S(i).IFdata(:, find(ismember(IFnames,'1_FarRedHigh_block bg')));
        S(i).FarRedHighcyt = S(i).IFdata(:, find(ismember(IFnames,'1_FarRedHigh_cyto ring')));
        S(i).FarRedHighPunctaArea = S(i).IFdata(:,contains(IFnames,'1_FarRedHigh_puncta area'));
        S(i).FarRedHighPunctaIntensity = S(i).IFdata(:,contains(IFnames,'1_FarRedHigh_puncta intensity'));
        S(i).FarRedHighPunctaNum = S(i).IFdata(:,contains(IFnames,'1_FarRedHigh_puncta number'));
...
%     S(i) = gateout_all(S(i),~(badNoise));
    ...
    if ~isempty(setting.apc)
        ...
        if setting.poiApc
            if setting.quiescentAnalysis
                [S(i).POI(:,2), badTracesAPC] = findAPCInact_C126(S(i).apcNuc, S(i).traceStats, ...
                    struct('postBuffer',5,'smooth',5,'cycling',0,...
                    'buff',1,'thresh',1,'preBuffer',1,'lowThresh',20, 'increase',5*1,'trunc',50, 'medfilt',0,'early', 25));
            else
...
%         S(i).crlNuc = fillTraceVals(S(i).crlNuc, S(i).traceStats, 5);
        ...
        if setting.poiCrl
            S(i).POI(:,3) = findCRL4Act_C169(S(i).crlAreaNorm,S(i).traceStats, ...
                struct('cycling',~setting.quiescentAnalysis,'low',.1,'smooth',3, ...
                'firstD',-.05,'secD',-.001,'early', 5,'falseCall', .8,'buff',0, 'postBuffer', 3,'decrease', 3*.05));
    ...
    if ~isempty(setting.sig)
        ...
        for n = 1:size(S(i).sigArea,1)
            ignore_norm = 1;
            S(i).sigNorm(n,:) =  S(i).sigNuc(n,:)./max(S(i).sigNuc(n,ignore_norm:end-1));
            S(i).sigSmooth(n,:) = nansmoothm(S(i).sigNuc(n,:), 5,'sgolay');
            S(i).sigNormSmooth(n,:) = nansmoothm(S(i).sigNorm(n,:), 5,'sgolay');
            S(i).sigDiff(n,:) = gradient(S(i).sigSmooth(n,:));
        end
        
        %Gate out cells that increase too high
        clear  highNoise;
        noiseThresh = .25;
        for n = 1:size(S(i).sigNuc,1)
            highNoise(n,1) = any((diff(S(i).sigNorm(n,:))) > noiseThresh & noiseMask);
        end
        S(i) = gateout_all(S(i),~( highNoise));
        
        if setting.poiCrl
           S(i).POI(:,3) = findCRL4Act(S(i).sigNorm,S(i).traceStats, ...
                struct('cycling',~setting.quiescentAnalysis,'low',.1,'smooth',5, 'smoothSec',6,'postBufferSec',1,...
                'firstD',0.01,'secD',-.001,'early', 10,'falseCall', .9,'buff',0, 'postBuffer', 6,'decrease', .02*6,'debug',0));
    ...
    if ~isempty(setting.PCNA)
        ...
        for n = 1:size(S(i).pcnaNuc,1)
           highTracestart(n,1) = S(i).filterArea(n,S(i).traceStats(n,1),4) > punctaThresh;
        ...
        S(i) = gateout_all(S(i),~( highNoise | highTracestart));
        
        if setting.poiPCNA
            for j = 1:size(S(i).filterArea,3)
                S(i).filterPOI(:,j) = getPCNAstart(S(i).filterArea(:,:,j),S(i).traceStats,...
                    struct('initoffset', 4,'thresh', 50, 'lowthresh',3,'debug',0));
            end
        end
    end
...
save(fullfile(setting.saveDir, setting.saveName),'S','-v7.3');
