...
setting.saveName = 'C130_data.mat';
setting.dataDir = 'F:\Data\C-Cdt1\Figures\Paper\ND-Cdt1-starve\CDK\C130';
    ...
    if ~isempty(setting.crl)
        ...
        if setting.poiCrl
            S(i).POI(:,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea,'traceStats', S(i).traceStats,'nuc',S(i).crlNuc, 'cyto', S(i).crlCyto), ...
                struct('cycling',~setting.quiescent,'smooth',5 ,'buff',10 , 'postBuffer', 5 ,'postBufferSec',0,...
                'firstD',0,'secD',-.005,'decrease', .025*5, 'minSlope', -.025, 'early', 25,'falseCall', .8, 'cytoCheck', 0,'minExp',100,...
                'trunk',.4,'debug',0));
