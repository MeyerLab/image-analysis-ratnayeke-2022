...
%%% Paths
experiment_name='C184-live-nikon';
image_drive = 'E:\4TB9\Data\';
savepath=['F:\Data\C-Cdt1\',experiment_name,'\Data_1frame\'];
imagepath=[image_drive,experiment_name,'\Raw_rename\'];
biaspath=[image_drive,experiment_name,'\Bias\'];
maskpath=[image_drive,experiment_name,'\Mask\'];
bgcmospath='';

%%% General parameters
liveFrame=108;   %Frame to start analyzing from
...
bgprctile = [25 25 25];
sigblur = [3 3 3];
localbgmeasure = [0 0 0];
ringcalc = [0 0 0];
ringthresh = [0 50 50];
punctacalc = 0;
punctaThresh = [125 150 200];
varThresh = [75 100 125];

%%% Segmentation parameters
firstsegmethod = 'log'; %Options: 'concavity', 'log', 'single adapt', 'double adapt', 'single', 'double'
...
if ~exist([savepath,'tracedata_',shot,'.mat'],'file')
    ...
    %%% Load images %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for i = 1:length(signals)
        raw{i} = double(imread([rawdir,signals{i},num2str(liveFrame),'.tif']));
        [height, width] = size(raw{i});
        if bgcmoscorrection
            raw{i} = (raw{i}-bgcmos);
        end
        if bias(i)
