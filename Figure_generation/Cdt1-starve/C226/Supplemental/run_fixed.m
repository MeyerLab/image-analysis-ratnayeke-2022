cd('F:\Data\C-Cdt1\C226-IF')
%% Rename IF 
% nikon_rename('N:\C235-IF1\20220117_135752_751',...
%      'N:\C235-IF1\Raw','COPY',false,'SINGLE_SITE',false)


%% Calculate Bias IF
% clear p
% p.biaspath='N:\C235-IF1\Bias';  % Output path for bias
% p.imagepath='N:\C235-IF1\Raw';  % Image directory. If nikon, single folder for all sites
...
% p.foreground_calc = 1;  % 1 for foreground calc (average foreground signal)
...
%% Setup IF images
clear settings
settings.data_path = 'F:\Data\C-Cdt1\C226-IF\Data\';      % Output location of mat files
settings.IF_imagesessions = {'I:\C226-IF1\'};    
...
settings.signals = {{'DAPI','FarRed'}};  % Each imaging session in a cell inside cell array
...
settings.localbg = {[0 0 ]};        % Measure local background in box around cell
...
settings.bgperctile = {[25 25]};  % Set bg as perctile for each channel
settings.frameIF=1;
 
Fixed_IF( settings, 6,3,10,1)
%% Run timelapse
rows = [5:6];
cols = [2:7];
sites = [1:36];
