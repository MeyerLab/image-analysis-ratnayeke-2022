...
if ~exist(fullfile(s.savepath,['tracedata_',shot,'.mat']),'file')
    ...
    for i = firstgoodindex:totalframes
        ...
        mask_props = regionprops(nuc_mask,raw{1},'Area','Solidity');
...
%         gate = isLarge & ~isMitotic & ~isDim;
        ...
        if i>firstgoodindex
            ...
            [tracedata,curdata,tracking,nuc_label] = ...
                adaptivetrack_NR(f,lastgoodframe,f,tracedata,curdata,tracking,real{1},nuc_label,jitters(f,:),trackparams,debugpackage);
            badframes(f) = 0;
            ...
            %{
           cropcoors = getcropcoors([width height ], reljitx, reljity);
           prev_label_test = prev_label(cropcoors(1,1):cropcoors(1,2),cropcoors(1,3):cropcoors(1,4));
           nuc_label_test = nuc_label(cropcoors(2,1):cropcoors(2,2),cropcoors(2,3):cropcoors(2,4));
           matchinglabel=prev_label_test.*nuc_label_test;
           tempframe=zeros(size(matchinglabel,1),size(matchinglabel,2),3);
           tempframe(:,:,1)=prev_label_test>0;
           tempframe(:,:,2)=nuc_label_test>0;
