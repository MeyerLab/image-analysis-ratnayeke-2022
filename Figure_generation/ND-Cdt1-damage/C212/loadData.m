...
setting.saveName = 'C126_data.mat';
setting.dataDir = 'F:\Data\C-Cdt1\Figures\Paper\ND-Cdt1-starve\Damage\C126';
        ...
        if setting.poiApc
            if setting.quiescent
                [S(i).POI(:,2), badTracesAPC] = findAPCInact(S(i).apcNuc, S(i).traceStats, ...
                    ...
                    'buff',1,'thresh',.25,'preBuffer',1,'lowThresh',20, 'increase',10*.4,'trunc',50, 'medfilt',0,'early', 25,'debug',0));
            else
    ...
    if ~isempty(setting.crl)
        ...
        if setting.poiCrl
            S(i).POI(:,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea,'traceStats', S(i).traceStats,'nuc',S(i).crlNuc, 'cyto', S(i).crlCyto), ...
                struct('cycling',~setting.quiescent,'smooth',5 ,'buff',10 , 'postBuffer', 3 ,'postBufferSec',0,...
                'firstD',0,'secD',-.005,'decrease', .04*3, 'minSlope', -.04, 'early', 25,'falseCall', .8, 'cytoCheck', 0,'minExp',100,...
                'trunk',.4,'debug',0));
